...
--
Following the definition of the Laplace transform, we have that: $$\mathcal{L}\{e^{5t}t\}=\int_0^\infty e^{-st}e^{5t}t\,dt=\int_0^\infty te^{-(s-5)t}\,dt$$ $$\lim_{a\to\infty} -\frac{te^{-(s-5)t}}{s-5}-\frac{e^{-(s-5)t}{(s-5)^2}}{s-5}\bigg|_0^a=\frac{1}{(s-5)^2}.$$
